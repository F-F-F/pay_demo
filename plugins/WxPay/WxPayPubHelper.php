 ...
 */
require("SDKRuntimeException.php");
require("WxPay.pub.config.php");
...
class OrderQuery_pub extends Wxpay_client_pub
{
    ...
    public function createXml()
    {
        try {
            ...
            $this->parameters["mch_id"] = WxPayConf_pub::$mchid;//商户号
            $this->parameters["nonce_str"] = $this->createNoncestr();//随机字符串
            $this->parameters["sign"] = $this->getSign($this->parameters, WxPayConf_pub::$wxkey);//签名
            return $this->arrayToXml($this->parameters);
        } catch (SDKRuntimeException $e) {
            die($e->errorMessage());
        }
...
class Refund_pub extends Wxpay_client_pub
{
    ...
    public function createXml()
    {
        try {
            ...
            } elseif (empty($this->parameters["out_refund_no"])) {
            ...
            } elseif (empty($this->parameters["refund_fee"])) {
...
class RefundQuery_pub extends Wxpay_client_pub
{
    ...
    {
        try {
            if (empty($this->parameters["out_refund_no"]) &&
                empty($this->parameters["out_trade_no"]) &&
                empty($this->parameters["transaction_id"]) &&
                empty($this->parameters["refund_id "])) {
...
class ShortUrl_pub extends Wxpay_client_pub
{
    ...
    {
        $this->parameters["long_url"] = $long_url;
        $this->postXml();
        $this->result = $this->xmlToArray($this->response);
...
class Wxpay_server_pub extends Common_util_pub
{
    ...
    public function checkSign()
    {
        $tmpData = $this->data;
        unset($tmpData['sign']);
        $sign = $this->getSign($tmpData, $this->key);//本地签名
...
class NativeCall_pub extends Wxpay_server_pub
{
    ...
    public function createXml()
    {
        if ($this->returnParameters["return_code"] == "SUCCESS") {
            if ($this->returnParameters["prepay_id"] == null) {
                throw new SDKRuntimeException("缺少Native支付二维码链接必填参数prepay_id！" . "<br>");
            }
            ...
            $this->returnParameters["sign"] = $this->getSign($this->returnParameters, $this->key);//签名
        }
        return $this->arrayToXml($this->returnParameters);
    }
...
class JsApi_pub extends Common_util_pub
{
    ...
    /**
     * 	作用：通过curl向微信提交code，以获取openid与token
     */
